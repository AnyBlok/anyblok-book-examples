...
class TestApiAddressesBase:
    ...
    def test_addresses_post_fail_bad_column(self, rollback_registry, webserver):
        ...
        fail = webserver.post_json(
            "/api/v1/addresses", [address_with_bad_column], status=400
        )
        assert fail.status_code == 400
        assert fail.json_body.get("status") == "error"
        assert fail.json_body.get("errors")[0].get("location") == "body"
    ...
    def test_addresses_post_fail_bad_value_type(self, rollback_registry, webserver):
        ...
        fail = webserver.post_json(
            "/api/v1/addresses", address_with_bad_value_type, status=400
        )
        assert fail.status_code == 400
        assert fail.json_body.get("status") == "error"
        assert fail.json_body.get("errors")[0].get("location") == "body"
    ...
    def test_address_put_fail_bad_path(self, rollback_registry, webserver):
        ...
        put_data = dict(first_name="Bob",)
        fail = webserver.put_json("/api/v1/addresses/x", put_data, status=400)
        assert fail.status_code == 400
        assert fail.json_body.get("status") == "error"
        assert fail.json_body.get("errors")[0].get("location") == "path"
    ...
    def test_address_put_fail_bad_value_type(self, rollback_registry, webserver):
        ...
        fail = webserver.put_json(
            "/api/v1/addresses/%s" % address.uuid, put_data, status=400
        )
        assert fail.status_code == 400
        assert fail.json_body.get("status") == "error"
        assert fail.json_body.get("errors")[0].get("location") == "body"
    ...
    def test_address_patch(self, rollback_registry, webserver):
        ...
        address = self.create_address(rollback_registry)
        patch_data = dict(first_name="Bob", last_name="Plop")
        response = webserver.patch_json(
            "/api/v1/addresses/%s" % address.uuid, patch_data,
        )
        assert response.status_code == 200
        assert response.json_body.get("uuid") == str(address.uuid)
        assert response.json_body.get("first_name") == "Bob"
    ...
    def test_address_patch_fail_bad_value_type(self, rollback_registry, webserver):
        ...
        fail = webserver.patch_json(
            "/api/v1/addresses/%s" % address.uuid, patch_data, status=400
        )
        assert fail.status_code == 400
        assert fail.json_body.get("status") == "error"
        assert fail.json_body.get("errors")[0].get("location") == "body"
